...
export const safeErrorMessage = (error: unknown, fallback: string): string => {
  const message = error && typeof error === 'object' && 'message' in error
    ...
    : '';
  if (/must be|required|invalid|contains invalid|no updatable|already|not assigned|not active|not found|cannot be|is already|only draft|only field-verified|only approved|must be submitted|can no longer|is already forwarded|no EMI amount/i.test(message)) {
    return message;
  }
  return fallback;
};
